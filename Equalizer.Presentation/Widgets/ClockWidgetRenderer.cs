...
public sealed class ClockWidgetRenderer : IWidgetRenderer
{
    public string WidgetTypeId => "clock";
    ...
    public void Render(object drawingContext, WidgetConfig config, double canvasWidth, double canvasHeight)
    {
        ...
        var settings = ClockWidgetSettings.FromConfig(config);
        var format = settings.Use24Hour 
        ...
        var timeText = _currentTime.ToString(format, CultureInfo.CurrentCulture);

        var typeface = new Typeface(new System.Windows.Media.FontFamily(settings.FontFamily), FontStyles.Normal, FontWeights.Light, FontStretches.Normal);
        ...
        if (settings.ShowShadow)
        {
            ...
            var shadowText = new FormattedText(timeText, CultureInfo.CurrentCulture, System.Windows.FlowDirection.LeftToRight,
                typeface, settings.FontSize, shadowBrush, VisualTreeHelper.GetDpi(WpfApplication.Current.MainWindow).PixelsPerDip);
            dc.DrawText(shadowText, new WpfPoint(x + 2, y + 2));
        }
