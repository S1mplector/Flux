...
namespace Flux.Application.Services;

public sealed class FluxService : IFluxService
{
    ...
    private async Task<VisualizerFrame> ComputeFrameInternalAsync(FluxSettings settings, CancellationToken cancellationToken)
    {
        try
        {
            int minSamples;
