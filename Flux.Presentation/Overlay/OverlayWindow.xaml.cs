...
{
    private readonly IFluxService _service;
    ...
    public OverlayWindow(IFluxService service, ISettingsPort settings, WidgetManager? widgetManager = null)
    {
        _service = service;
        _settings = settings;
    ...
    private void RenderLinearBars(DrawingContext dc, VisualizerFrame vf, float[] data, FluxSettings s, double width, double height, double spacing)
    {
    ...
    private void RenderCircular(DrawingContext dc, VisualizerFrame vf, float[] data, FluxSettings s, double width, double height)
    {
        if (data.Length == 0) return;
    ...
    private bool TryGetSettingsSnapshot(out FluxSettings? settings)
    {
